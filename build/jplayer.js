...
(function (window, document, $, $p) {
  ...
  $p.newModel(
    {
      ...
      applyMedia: function (destContainer) {
        ...
        $.extend(
          true,
          ref._hlsjsPlatformConfig,
          { src: "/MISSING_PATH_TO_HLSJS_LIB/", initVars: {} },
          msePlatformConfig.hlsjs
        );
        ...
        if (window.Hls && typeof window.Hls.isSupported === "function") {
          ...
          // load hls.js
          $p.utils
            .getScript(ref._hlsjsPlatformConfig.src, {
              cache: true,
            })
      ...
      applySrc: function () {
        var ref = this,
          media = ref.getSource(),
          wasAwakening = ref.getState("AWAKENING"),
          hlsConfig = getHlsConfig(ref._hlsjsPlatformConfig.initVars, media[0]);
      ...
      updateAvailableDynamicStreamsQualities: function (data) {
        var dynamicStreams = data.levels,
          ...
          keyName = "",
          isAudioOnly = false,
          showAudioOnly = this.pp.getConfig(
            "dynamicStreamShowAudioOnlyQualities"
          ),
          avKeyFormat = this.pp.getConfig(
            "dynamicStreamQualityKeyFormatAudioVideo"
          ),
          aoKeyFormat = this.pp.getConfig(
            "dynamicStreamQualityKeyFormatAudioOnly"
          ),
          dpc = this.pp.getConfig(
            "dynamicStreamQualityKeyBitrateRoundingDecimalPlacesCount"
          ),
          bitrate = 0,
          bitrateKbps = 0,
          bitrateMbps = 0,
      ...
      switchDynamicStreamIndex: function (index) {
        ...
        if (
          ...
          (index === this.getCurrentDynamicStreamIndex() &&
            !this.getAutoDynamicStreamSwitch()) ||
      ...
      getMaxAllowedDynamicStreamIndex: function () {
        if (
          this.getAutoDynamicStreamSwitch() &&
          this._hlsjs.autoLevelCapping >= 0
        ) {
      ...
      setMaxAllowedDynamicStreamIndex: function (val) {
        if (
          !isNaN(val) &&
          val !== this.getMaxAllowedDynamicStreamIndex() &&
          val >= 0 &&
          val < this.getNumDynamicStreams()
        ) {
      ...
      getDynamicStreamingStatus: function (name) {
        if ($p.utils.logging) {
          ...
          $p.utils.log(
            ...
            ", current index: ",
            this.getCurrentDynamicStreamIndex(),
            ", max allowed index: ",
            this.getMaxAllowedDynamicStreamIndex(),
            ", num streams: ",
            this.getNumDynamicStreams(),
            ", auto:",
            this.getAutoDynamicStreamSwitch()
          );
          ...
          for (var index in streams) {
            if (
              streams.hasOwnProperty(index) &&
              streams[index].bitrate !== undefined
            ) {
              name =
                index +
                " dimensions: " +
                streams[index].width +
                "x" +
                streams[index].height +
                " | bitrate: " +
                streams[index].bitrate +
                " | streamName: " +
      ...
      setQuality: function (quality) {
        ...
        if (this._isDynamicStream === true) {
          this.switchDynamicStreamIndex(
            quality == "auto" ? -1 : this.availableQualities[quality]
          );
        }
      },
    },
  ...
  $p.newModel(
    {
      ...
      applyMedia: function (destContainer) {
        ...
        if (window.Hls && typeof window.Hls.isSupported === "function") {
          ...
          // load hls.js
          $p.utils
            .getScript(ref.pp.getConfig("platformsConfig").mse.src, {
              cache: true,
            })
            .done(hlsJsLoadSuccess)
            .fail(hlsJsLoadFailed);
  ...
  function getHlsConfig(config, media) {
    var headers = Object.entries(media.headers || {});
    if (headers.length > 0) {
  ...
  $p.newModel(
    {
      ...
      applyMedia: function (destContainer) {
        ...
        this._showAudioOnly = this.pp.getConfig(
          "dynamicStreamShowAudioOnlyQualities"
        );
      ...
      _setDashJsConfig: function (dashjsConfig) {
        ...
        Object.keys(dashjsConfig).forEach(function (configKey) {
          ...
          try {
            ...
            $p.utils.log(
              "DASHJS config setting failed on: ",
              configKey,
              configVal,
              error
            );
      ...
      applySrc: function () {
        var file = this.getSource()[0],
          headers = Object.entries(file.headers || {}),
          fileDrmConfig = Array.isArray(file.drm) ? file.drm : [],
          drmConfig = this.pp.getConfig("drm") || {}, // item or global
          availableDrmConfig = $p.utils.intersect(
            fileDrmConfig,
            Object.keys(drmConfig)
          ),
        ...
        // Add custom headers
        if (headers.length > 0) {
          this._dashjs.extend("RequestModifier", function () {
            return {
      ...
      _fetchDashJs: function (cb) {
        ...
        $.extend(
          true,
          ref._dashjsPlatformConfig,
          { src: "/MISSING_PATH_TO_DASHJS_LIB/", initVars: {} },
          msePlatformConfig.dashjs
        );
        ...
        if (typeof window.dashjs === "object") {
          ...
          $p.utils
            .getScript(ref._dashjsPlatformConfig.src, {
              cache: true,
            })
            .done(function () {
              if (typeof window.dashjs === "object") {
      ...
      _getQualityList: function () {
        var avKeyFormat = this.pp.getConfig(
            "dynamicStreamQualityKeyFormatAudioVideo"
          ),
          aoKeyFormat = this.pp.getConfig(
            "dynamicStreamQualityKeyFormatAudioOnly"
